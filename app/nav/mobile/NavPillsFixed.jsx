...
function NavPillsFixed() {
  ...
  const shouldDisplay =
    activeCategory && getCategorySections(activeCategory).length > 1;

  return (
    <>
      {!isBreakpoint("md") &&
        shouldDisplay &&
        scrollY > navMobileBounds.height - 10 &&
        fixedNavigationEnabled && (
          <div
            id={`fixed-nav-pills-wrapper`}
            className={`fixed-nav-pills-wrapper ${utils.strIf(!outsideOfBonds, `fixed-nav-pills-wrapper-hidden`)}`}>
